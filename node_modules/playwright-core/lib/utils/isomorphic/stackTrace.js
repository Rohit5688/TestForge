...
function captureRawStack() {
  ...
  try { Error.stackTraceLimit = 50; } catch { /* read-only in some Node environments */ }
  const error = new Error();
  const stack = error.stack || "";
  try { Error.stackTraceLimit = stackTraceLimit; } catch { /* read-only in some Node environments */ }
  return stack.split("\n");
}
